...
目前版本：`v0.0.29`。各操作連續耗盡重試時會依序執行局部處理、階段安全恢復與遊戲
...
清除前仍必須依序證明 Attack／HP 親代篩選與頂尖／量產自動放置全部完成。
...
- S13 與主力使用相同的完整孵蛋流程「孵化 → Attack → HP 親代篩選 → 頂尖 → 量產 → 收集 → 洞穴淘汰」，可在孵化倒數期間狩獵；差別只在容量：S13 為 `180/200`，主力為 `330/350`。新手專用流程（所有 → 自動放置一次 → 收集一次）仍保留在 CLI，供其他新開帳號使用。
