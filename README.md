...
目前版本：`v0.0.8`。親代與候選數值需連續兩幀一致，HP 必須是 10 的倍數，並保留
HP 升級 10–30、攻擊升級 1–3、速度有效值 1–150 的防呆；校正失敗時拒絕替換並保存
親代採證。完整巢穴管理會累積孵化 8 顆後執行，少量新蛋不再連續重跑四種篩選；洞穴
到達實際淘汰線時仍會立即處理，且清除前必須證明 Attack／HP／Top／Mass 全部完成。
這一版同時保留 v0.0.7 的多 Bot Dashboard、重複登入自動重啟、CMD 即時日誌與 runtime
修復，以及低效能電腦的孵化恢復、
