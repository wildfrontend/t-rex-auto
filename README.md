...
目前版本：`v0.0.11`。孵蛋流程首次讀到總恐龍數時先完成一次巢穴篩選，之後以最近一次
篩選的總數為基準，每增加 20 隻才重跑篩選；總數達 `330/350` 時，篩選後進洞穴清理。
收集所有蛋固定沿用原本的 My Nest 流程，不再操作主畫面的收集蛋按鈕。狩獵地圖與自動
放置下拉選單也已依新版 UI 重新校正。任何座標連續兩次點擊後畫面完全沒變時，Bot 會改
按返回鍵脫困，不再無限重試同一點。親代與候選數值
仍需連續兩幀一致，HP 必須是 10 的倍數，並保留 HP 升級 10–30、攻擊升級 1–3、
速度有效值 1–150 的防呆；清除前仍必須證明 Attack／HP／Top／Mass 全部完成。
