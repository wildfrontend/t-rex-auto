...
目前版本：`v0.0.5`。這一版會保存洞穴裁切畫面的有效容量、要求連續兩幀容量一致才
採信，並以字形封閉圈校正 `5/6/8` OCR；巢穴掃描依階段縮小範圍，領取後的未完成蛋
詳細頁也能直接驗證成功。親代素質仍有 HP 升級 10–30、攻擊升級 1–3、速度有效值
1–150 的防呆，超出範圍時拒絕替換；同時保留低效能電腦的孵化恢復、
