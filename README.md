...
目前版本：`v0.0.10`。任何座標連續兩次點擊後畫面完全沒變，就改按返回鍵脫困，不再
無限重試同一點。前一版修正 HP 小字體 `0` 被讀成 `6`，並在相同親代校正錯誤連續
三輪、或 My Nest 開啟耗盡四次驗證重試時封鎖孵化；混合模式會回去狩獵，不再讓
孵化恢復空轉數小時。孵化點擊無反應時也會先重新檢查 `N/350`，確認容量未滿才重試。
親代與候選數值仍需連續兩幀一致，HP 必須是 10 的倍數，並保留 HP 升級 10–30、
攻擊升級 1–3、速度有效值 1–150 的防呆。完整巢穴管理會累積孵化 8 顆後執行；洞穴
到達實際淘汰線時會立即處理，且清除前必須證明 Attack／HP／Top／Mass 全部完成。
