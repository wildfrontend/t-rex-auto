...
目前版本：`v0.2.5-android-usb-experiment`。這是與正式版隔離的 Android USB
實機實驗，只接受唯一 USB 實機，多台或沒有實機時都會安全停止：

- macOS 雙擊 `start-bot.command` 會先部署到 `.runtime-macos/app`，再從獨立副本啟動。
- ADB 設為 `auto` 且 `require_usb: true`，不會回退去操作 BlueStacks。
- 多台 USB 實機或多台模擬器同時可用時不會猜測，需明確設定裝置序號。
...
- 遊戲使用直向全螢幕；辨識會正規化為 `900 × 1600`。

可使用 USB Android 實機或 BlueStacks Air：

- 實機：開啟「開發人員選項 → USB 偵錯」，以 USB 連接 Mac，並在手機上允許這台
  電腦的偵錯授權。
- 此實驗版不操作 BlueStacks；正式版仍保留在原本的 `main` worktree。

啟動 Dino Mutant 並停在主地圖或採集地圖，接著在 Finder 雙擊根目錄的：
...
4. Bot 預設會自動選擇唯一 USB 實機；沒有實機時才使用 BlueStacks。
...
auto（BlueStacks 備援：127.0.0.1:5555）
```
