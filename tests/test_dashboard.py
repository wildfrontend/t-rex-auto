...
def test_dashboard_can_update_mutation_safety_ranges(tmp_path: Path) -> None:
    app = tmp_path / "app"
    app.mkdir()
    config_path = app / "config.json"
    config_path.write_text(
        json.dumps({"hatch": {"stat_upgrade_guards": {"speed": {"max_value": 150}}}}),
        encoding="utf-8",
    )
    controller = DashboardController(tmp_path, app / "logs", config_path=config_path)

    result = controller.update_mutation_guards(
        instance_id="main",
        hp_min_delta=30,
        hp_max_delta=70,
        attack_min_delta=3,
    ...
    assert result["mutation_guards"] == {
        "hp_min_delta": 30,
        "hp_max_delta": 70,
        "attack_min_delta": 3,
        "attack_max_delta": 7,
    }
    assert config["hatch"]["stat_upgrade_guards"]["hp"] == {
        "min_delta": 30,
        "max_delta": 70,
        "multiple_of": 10,
    }
    assert config["hatch"]["stat_upgrade_guards"]["attack"] == {
        "min_delta": 3,
        "max_delta": 7,
    }
...
def test_dashboard_rejects_invalid_mutation_safety_ranges(tmp_path: Path) -> None:
    app = tmp_path / "app"
    app.mkdir()
    config_path = app / "config.json"
    ...
    with pytest.raises(ValueError, match="multiples of 10"):
        controller.update_mutation_guards(
            instance_id="main",
            hp_min_delta=35,
            hp_max_delta=70,
            attack_min_delta=3,
...
def test_dashboard_updates_mutation_safety_ranges_over_api(tmp_path: Path) -> None:
    app = tmp_path / "app"
    app.mkdir()
    config_path = app / "config.json"
    config_path.write_text("{}", encoding="utf-8")
    ...
    with server:
        update = Request(
            f"{server.url}/api/control/set-mutation-guards",
            data=json.dumps(
                {
                    "hp_min_delta": 30,
                    "hp_max_delta": 70,
    ...
    assert result["mutation_guards"]["attack_max_delta"] == 7
    config = json.loads(config_path.read_text(encoding="utf-8"))
