...
cp -a "${project_root}/使用教學.md" "${runtime_root}/README-TW.md"
cp -a "${project_root}/.agents" "${runtime_root}/"

# Never ship bytecode generated by the host development Python.
find "${runtime_app}/src" -type f \( -name "*.pyc" -o -name "*.pyo" \) -delete
find "${runtime_app}/src" -depth -type d -name "__pycache__" -empty -delete
